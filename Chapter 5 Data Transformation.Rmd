...
Idea: create a variable `timeOnAir` to measure the flight time, and create a new dataset `flights_Testimate` that contains `timeOnAir`.
...
0. Just list all the variables to select 
```{r}
flights %>%
...
2. `ends_with` (inefficient in our case)
```{r}
flights %>%
...
3. Fancier (based on the specific arrangment of variables in our dataset)
```{r}
