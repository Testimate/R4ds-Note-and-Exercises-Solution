...
__Testimate: Displays number of cases within each group you defined by `group_by()`. In this case, if we are interested in delay performance for each plane, which is identified by their tail number (`group_by(tailnum)`) ,then under column of `n` shows the number of flights each plane took. 
